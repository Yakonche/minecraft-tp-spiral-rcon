...
class RawInput:
    def __init__(self, stream):
        self.stream = stream; self.fd = stream.fileno(); self.old=None
    def __enter__(self):
...
def _find_latest_save(conf: dict) -> str | None:
    e = conf["exploration"]; sd = conf.get("save_dir", "saves")
    player = e["player"]; dim = _dim_short(e["dimension"])
    chunks = int(e["chunks"]); sx = int(e["spawn_x"]); sz = int(e["spawn_z"]); y = int(e["y"])
...
def _upd_right_widths(sx, sy, sz):
    ...
    _wxR = max(_wxR, len(sx)); _wyR = max(_wyR, len(sy)); _wzR = max(_wzR, len(sz))
...
def _log_panel(rows, width):
    ...
    for left_markup, right_markup in rows:
        lt = Text.from_markup(left_markup); lt.no_wrap=True
        rt = Text.from_markup(right_markup); rt.no_wrap=True
        left_texts.append(lt); right_texts.append(rt)
...
def _controls_block():
    l1 = Text.assemble(
        ("Déplacement", "bold cyan"), (" : ", "dim"), ("→←↑↓/ZQSD", "khaki1"),
        (" | ", "dim"), ("Hauteur", "bold cyan"), ("(Y=16)", "dim"), (" : ", "dim"), ("A/E", "khaki1"),
        (" | ", "dim"), ("Quitter", "bold cyan"), (" : ", "dim"), ("Esc", "khaki1"),
    )
    l2 = Text.assemble(
        ("Saut en chunks", "bold cyan"), (" : ", "dim"), ("+/-", "khaki1"), (" (×2/÷2)", "dim"),
        (" | ", "dim"), ("R", "bold cyan"), (" : ", "dim"), ("Reset", "khaki1"),
        (" | ", "dim"), ("Dimension", "bold cyan"), (" : ", "dim"), ("i", "khaki1"),
    )
    return [l1, l2]
        ...
        while True:
            ...
            while time.time() - start < 0.8:
                ...
                if k == "UP" or k.lower() == "z":
                    z -= step_chunks*16; tp_count += 1; moved = True; break
