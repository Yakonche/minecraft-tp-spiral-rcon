import sys, subprocess, re, time, os, socket, struct, random
...
def _resolve_playerdata_dir(playerdata_dir: str) -> str:
    ...
    if os.path.isfile(os.path.join(base, "server.properties")):
        try:
            with open(os.path.join(base, "server.properties"), "r", encoding="utf-8", errors="ignore") as f:
                for ln in f:
        ...
        try:
            ...
            if p.returncode == 0 and p.stdout:
                ...
                for ln in p.stdout.splitlines():
                    m1 = re.match(r"^Joueur:\s*(.+)\s*$", ln)
                    if m1:
                        cur = m1.group(1).strip(); continue
                    m2 = re.match(r"^\s*Monde\s*:\s*([^,]+)", ln)
                    if m2 and cur:
                        out[cur] = m2.group(1).strip(); cur = None
                if out:
                    setter(out)
        except Exception:
            pass
    ...
    while not stop_event.is_set():
        try:
            ...
            for ln in (p.stdout or "").splitlines():
                m = re.match(r"^Joueur:\s*(.+)\s*$", ln)
                if m:
                    if cur: players.append(cur)
                ...
                if m: cur["score"] = m.group(1)
                m = re.match(r"^\s*Monde\s*:\s*([^,]+),\s*Gamemode:\s*(.+)\s*$", ln)
                if m:
                    cur["dim"] = m.group(1).strip()
                    cur["gm"] = m.group(2).strip()
                m = re.match(r"^\s*Position\s*:\s*(.+)\s*$", ln)
                if m: cur["pos"] = m.group(1).strip()
            if cur: players.append(cur)
            if players: setter(players)
        except Exception:
            pass
        for _ in range(int(interval_s * 10)):
            if stop_event.is_set(): break
...
def poll_stats_dims(stop_event, nbt_py, playerdata_dir, usernamecache, interval_s, setter_stats, setter_dims, only_on_change=True):
    last_mtime = 0.0
    while not stop_event.is_set():
        try:
            real_dir = _resolve_playerdata_dir(playerdata_dir)
    ...
    while not stop_event.is_set():
        try:
            ...
            if not names:
                ...
                for n in names:
                    p={"name":n}
                    if n in hp: p["health"]=re.findall(r"[-+]?\d+(?:\.\d+)?",hp[n])[0]
                    if n in hunger: p["hunger"]=re.findall(r"-?\d+",hunger[n])[0]
                    if n in lvl: p["lvl"]=re.findall(r"-?\d+",lvl[n])[0]
                    if n in gm: p["gm"]=_to_gm(gm[n])
                    if n in dim: p["dim"]=dim[n]
                    if n in pos: p["pos"]=_fmt_pos(pos[n])
                    players.append(p)
                setter_stats(players)
                setter_dims({n:dim.get(n,"") for n in names if n in dim})
        except Exception:
            pass
        for _ in range(int(interval_s*10)):
            if stop_event.is_set(): break
...
def _read_usernamecache_names(usernamecache):
    ...
    try:
        import json
        with open(usernamecache,"r",encoding="utf-8",errors="ignore") as f:
            data=json.load(f)
        if isinstance(data, list):
            for it in data:
                n=it.get("name")
                if isinstance(n,str) and n: names.add(n)
        elif isinstance(data, dict):
            for k,v in data.items():
                if isinstance(v, str) and v and len(v)<=16: names.add(v)
...
def poll_stats_hybrid(stop_event, rcon, nbt_py, playerdata_dir, usernamecache, interval_s, setter_stats, setter_dims, nbt_refresh_s=30):
    cache_nbt={}
    last_mtime=0.0
    last_nbt_time=0.0
    first=True
    while not stop_event.is_set():
        ...
        try:
            ...
            hunger=_parse_data_map(_rcon_cmd(rcon,"execute as @a run data get entity @s foodLevel")) if names_online else {}
            lvl=_parse_data_map(_rcon_cmd(rcon,"execute as @a run data get entity @s XpLevel")) if names_online else {}
            gm=_parse_data_map(_rcon_cmd(rcon,"execute as @a run data get entity @s playerGameType")) if names_online else {}
            dim=_parse_data_map(_rcon_cmd(rcon,"execute as @a run data get entity @s Dimension")) if names_online else {}
            ...
            for n in sorted(names_all):
                ...
                if n in dim:
                    dv=dim[n].strip('"')
                    if dv.startswith('"') and dv.endswith('"'): dv=dv[1:-1]
                    p["dim"]=dv.strip('"')
                if n in pos:
                    p["pos"]=_fmt_pos(pos[n])
                if "dim" in p: dims[n]=p["dim"]
                players.append(p)
            setter_stats(players)
            setter_dims(dims)
        except Exception:
